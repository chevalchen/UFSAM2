...
class AverageMeter:
    ...
    def __init__(self, dataset_name, class_ids, num_classes=-1):
        ...
        self.class_counter = torch.zeros(self.nclass).cuda()
...
class Evaluator:
    ...
    @classmethod
    def classify_prediction(cls, pred_mask, batch, device: torch.device):
        gt_mask = batch.get('query_mask').to(device)
        ...
        if query_ignore_idx is not None:
            query_ignore_idx = query_ignore_idx.to(gt_mask.device)
