...
class CheckpointValidationTest(unittest.TestCase):
    def setUp(self):
        self.model_state = {
            ...
            "post_memory_calibrator.residual_head.weight": FakeTensor(4, 4),
        }
    ...
    def test_rejects_pmc_contamination(self):
        checkpoint = {
            "sam.encoder.adapter.down.weight": FakeTensor(2, 4),
            "sam.encoder.adapter.up.weight": FakeTensor(4, 2),
            "post_memory_calibrator.residual_head.weight": FakeTensor(4, 4),
        }
        with self.assertRaisesRegex(RuntimeError, "must not contain AV-PMC"):
            validate_sansa_base_state(self.model_state, checkpoint)
