...
- Stage A PRE-FREEZE 候选（2026-07-27，`DIAGNOSTIC_ONLY`）：服务器在 clean detached SHA `c0568948263e99e783a6c5842931c6fef7b23a19` 上生成 COCO-20i fold 0、1-shot、seed 0 的 candidate manifest，包含 train/calibration/validation `1200/600/600` episodes，SHA256 为 `c0abf8545a07800e4f56cb4f83afd3738553dda87d1223855d46891156a23de9`。三个 partitions 之间 query/support identity overlap 为 0，均覆盖全部 60 个 base classes；每类 episode 数范围依次为 train `8–35`、calibration `3–20`、validation `5–19`。该 manifest 当前只存在于服务器临时目录，尚未进入 Git，因此不是 formal input。
- Stage A timing 候选（2026-07-27，`DIAGNOSTIC_ONLY`）：30-step operator probe 测得约 `0.546 s/optimizer step`、`0.455 s/validation episode`，单卡 peak GPU memory `3293 MiB`。候选 frozen exposure 为每 config `10 epochs / 12000 optimizer steps`，每 epoch 使用相同 1200 train episodes 并在相同 600 validation episodes 上选择 best checkpoint；预计每 config 约 `2.6–3.0 h`，六个预注册 projection-width/residual-scale configs 在两张独立 GPU 上按三波并行约 `7.8–9.0 h`。该 exposure 尚未冻结，未执行任何 formal architecture-search run。
- 会话与交接状态：当前仍由 Idea/Research Decision 会话负责，EXP-001 总状态保持 `DRAFT`。服务器下一步仅将上述 exact candidate manifest 与六配置 resolved Stage A config 作为 tracked inputs 提交并推送到 `exp/EXP-001-av-pmc`；主机随后核对 hashes、补齐 formal SHA，并在本账本明确记录 `Stage A: FROZEN`。只有完成该 commit 后，才移交新的 Experiment 会话按文档执行 Stage A `B0/B1/B8`，不得依赖旧会话上下文或在运行会话中改变设计。
- 固定 episode 与 validation contract：正式训练入口必须提供 `--episode_manifest`，随机未追踪 episodes 会被拒绝。manifest 同时包含互斥的 `train`、`calibration`、`validation` partitions；训练仅使用 `train`，每个 epoch 仅用 `validation` 计算 matched `B0/treatment/B8` 并按 treatment mIoU 保存 best checkpoint。`calibration` 保留给后续 threshold/budget selection，不参与 best-checkpoint 选择。dataset-bound candidate 已在服务器生成，但其 exact bytes 与 resolved config 仍须通过 Git 回传并在本账本绑定 tracked hashes 后才能冻结。
