...
- 本地验证状态：8 个相关 Python 文件通过 AST syntax check；默认 Python 与 Codex bundled Python 均缺少 PyTorch，因此未在本地执行 tensor/model smoke。该项属于环境缺失，不是实验结果；首次 server run 前必须完成 import、dummy tensor、checkpoint load 与单 episode smoke。
