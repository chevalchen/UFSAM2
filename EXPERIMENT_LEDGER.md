...
- EXP-001 主机训练前基础设施 SHA：`e89a189`；该提交修复 SANSA adapter-only checkpoint load contract，并实现 identity-checked episode manifest/replay、互斥 base-train/base-calibration/base-validation partitions、held-out validation 与 best-checkpoint selection。它仍需服务器 PyTorch/data/checkpoint smoke，不能直接视为 formal implementation SHA。
...
- 本地验证状态（2026-07-26）：checkpoint contract 与 episode manifest/replay 共 9 个纯 Python tests 通过，11 个相关 Python 文件通过 AST syntax check。测试覆盖 adapter-only 接受、adapter 缺失/shape mismatch/AV-PMC contamination 拒绝、manifest deterministic generation/JSON round-trip、RNG restoration、identity drift 与跨 partition image overlap 拒绝。默认 Python 与 Codex bundled Python 均缺少 PyTorch，因此尚未执行真实 tensor/model/checkpoint smoke；该项必须在服务器完成，当前没有 observed result。
