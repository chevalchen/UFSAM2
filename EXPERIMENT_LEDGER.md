...
- EXP-001 formal implementation/control SHA：Stage A 为 `7ab637758698ddb19fdacf6c17df1f09446bec26`；Stage B 为 `d2e841baa5810b3f76ea12d17081bb0a36490c81`；Stage C 因冻结的 Stage B 停止规则未启动，记为 `N/A`。
- EXP-001 主机训练前基础设施 SHA：`e89a189`，Python 3.8 运行时兼容补丁 SHA：`d477b29`；前者修复 SANSA adapter-only checkpoint load contract，并实现 identity-checked episode manifest/replay、互斥 base-train/base-calibration/base-validation partitions、held-out validation 与 best-checkpoint selection，后者确保这些新增入口可在 SANSA 常用的 Python 3.8 环境解析。服务器 Stage A/B formal execution 均已完成，provenance、matched controls 与 artifacts 已核验；Stage B 触发预注册 `DROP_AND_STOP_EXP_001`，Stage C 不再冻结或执行。
- 当前实验分支：`exp/EXP-001-av-pmc`；EXP-001 已完成 Stage A/B formal screening 并裁决为 `DROP`。Stage C 未启动；AV-PMC feature code 不进入 accepted branch，分支与外部 artifacts 仅作可追溯研究记录。
- 已接受模块：无
- 首个候选实验：`EXP-001 — AV-PMC`（已裁决 `DROP`）
...
- 状态：EXP-001 总体为 `DECIDED / DROP`；`Stage A: EVALUATED / PASS_TO_STAGE_B`；`Stage B: EVALUATED / FAIL`；`Stage C: NOT_STARTED / STOPPED_BY_CONTRACT`。
- 证据标签：Stage A operator/headroom 与 Stage B matched evaluation 为 `VERIFIED`；B8/B8S 为 `ORACLE`；完整 AV-PMC action-specific gating 主张经本实验反证，未获接受。
...
- 当前实现状态：已在 `exp/EXP-001-av-pmc` 上以 `000d450` 为 authority-import parent 完成 Stage A/B AV-PMC integration。Stage B implementation SHA `d2e841baa5810b3f76ea12d17081bb0a36490c81` 包含 deterministic hard top-area、B2/B3/B4E/B4R/B7S/B8S cached-feature matched decoding、calibration budget selection、per-episode outputs、dense AUPRC、negative-repair/worst-decile guardrails 与 paired bootstrap。MTP、BRM、support aggregation、hflip 和 episode action-value gate 均未接入 Stage B 活动路径。因 Stage B 已按冻结规则失败，不再实现 Stage C 的 episode-level `B5/B6` routing harness；现有 feature code 不进入 accepted branch。
...
- Stage B formal execution（2026-07-29）：Experiment 会话在 clean detached SHA `d2e841baa5810b3f76ea12d17081bb0a36490c81`、worktree `/tmp/ufsam2-exp001-stageb-formal.mf5wS6` 上完成 `DIAGNOSTIC_ONLY` server smoke、两项正式训练及 matched evaluation；17 个 repository tests 通过。环境为 Python 3.10.19、PyTorch 2.5.1+cu121、CUDA 12.1、2×RTX 3090。未发现 traceback、OOM、NaN、RuntimeError 或 provenance mismatch；全部进程已退出，Stage C 未创建或启动。smoke manifest SHA256 为 `0d13fe7663761efe407497328975369e9e806ba4a25c949fdd4fc25f3e8c6afc`，只证明执行链路，不作为科学数值证据。
...
- Stage B calibration：B2 在 area budgets `{0.10,0.25,0.50,1.00}` 的 calibration mIoU 依次为 `73.011/73.176/72.318/72.889`。`q=0.10` 距最高值仅 `0.165` point，故按冻结的 minimum-within-0.2 规则选择 `q*=0.10`；该选择有效且未触发 `q*=1.0` 停止条件。
...
- Stage B supporting diagnostics：`B2−B1 = −0.797` aggregate mIoU，`B8S−B2 = +1.617`；dense positive-benefit AUPRC 为 `0.7278`，B2 negative-repair rate 为 `56.83%`。B2/B0 worst-decile episode IoU 分别为 `13.356/13.458`。FB-IoU、worst-decile、exact-area 与 full-area B1 reproduction guardrails/sanity checks 全部通过。aggregate class-mIoU 与 paired episode mean 使用不同权重，故同一 comparison 的点估计可不同；阶段裁决同时遵循合同明确列出的 aggregate point-effect stop rule 与 paired CI gate。
- Stage B 阶段裁决：`FAIL / DROP_AND_STOP_EXP_001`。B2 对 B4E 与 B4R 的 aggregate point effects 分别为 `−0.251/−0.260`，已经直接触发“任一 point effect 非正即 DROP”的冻结条件；此外，`B2−B0` 未达到 `+0.5`，且全部必需 paired CI 下界均不大于 0。该结果不满足 `INCONCLUSIVE` 的前提，因为并非所有 point effects 为正。guardrails、provenance 与 sanity 全部通过，因此这是 learned action-specific spatial selection 的科学失败，不是环境、执行或实现链路故障。
- Stage B 解释边界：Stage A 仍认证 frozen always-on operator 在本 setting 的平均改善 `+0.873 mIoU`；Stage B 的 `B8S−B2 = +1.617` 与 AUPRC `0.7278` 只说明 dense oracle-proxy headroom/排序信号存在，不足以证明 learned B2 能把该信号转化为优于 random、entropy 或 generic-risk 的最终 segmentation。不得 post-hoc 改用 `q=0.25`、重选 checkpoint、改变 teacher/head/loss 或直接启动 Stage C 来挽救 EXP-001；任何实质变化必须创建新 Experiment ID。
- 会话与交接状态：Stage B Experiment 会话已经完成任务并将结果交回 Idea/Decision 会话。EXP-001 不再有服务器 formal execution 待办，Stage C 明确停止。后续若仅保留 unconditional post-memory repair，或改变 routing/teacher/operator，必须先在 Idea/Decision 阶段以新 Experiment ID 冻结独立假设；为满足 30 天 portfolio 主干目标，默认优先转入下一候选模块，而不继续追加 EXP-001 ablation。
...
- Config / checkpoint / episode hashes：Stage A config 与 episode hashes 见上述 frozen records；Stage B research contract、execution config、formal SHA、frozen operator 与 formal result hashes 见 Stage B records。official COCO-20i fold-0 adapter SHA256 为 `b02b96f30ee558c37ef4fc5f889e90772dafb117e42b3fcf0945000220bc57e5`，SAM2-Large base weights SHA256 为 `7442e4e9b732a508f80e141e7c2913437a3610ee0c77381a66658c3a445df87b`。formal run 未使用空 checkpoint path。Stage C inputs 因预注册停止规则记为 `N/A`，不得为 EXP-001 补冻。
- Datasets / folds / seeds：Stage A 与 Stage B 均冻结为 strict-FSS COCO-20i fold 0、1-shot、training seed 0，并使用上述互斥 train/calibration/validation episodes；Stage B random control seeds 固定为 `{0,1,2}`。one-seed screening 只用于主干阶段裁决，不单独满足最终多 seed `KEEP`。EXP-001 的 Stage C 与后续 cross-dataset confirmation 均因 Stage B hard-stop 取消；任何新实验若进入 PASCAL-5i/COCO-20i 1-shot/5-shot confirmation，仍须预先冻结。PASCAL-Part/PACO-Part 仅作为单独标注的 secondary part track。
...
- Result run IDs 与 paired summary：Stage A 六个 operator runs 见 Stage A 表；Stage B formal runs 为 `stage_b_b2_benefit_seed0`、`stage_b_b4r_risk_seed0` 与 `stage_b_spatial_matched_eval_seed0`。Stage A 所选 operator 的 `B1−B0 = +0.873 mIoU`；Stage B 在 `q*=0.10` 时，primary selection `B2−mean(B3)` aggregate 为 `+0.032`、paired mean 为 `−0.149 [−0.559,+0.271]`，primary specificity `B2−B4R` aggregate 为 `−0.260`、paired mean 为 `+0.052 [−0.376,+0.454]`。
- 决策：`DROP`。EXP-001 认证了一个有平均 headroom 的 unconditional operator，但 action-specific spatial routing 未在 matched area/compute 下优于 generic controls，且触发预注册 Stage B hard-stop。停止 Stage C；不得把 EXP-001 feature code 合入 accepted branch。若未来把 unconditional operator 作为独立方法，或修改 teacher/head/routing 后重试，必须以新 Experiment ID `REVISE`，不得改写本次结论。
- Artifact URI/path 与 SHA256：Stage A root 为 `/data6/chensq/UFSAM2_runs/EXP-001/stage_a`。Stage B root 为 `/data6/chensq/UFSAM2_runs/EXP-001/stage_b`；matched evaluation 的 `run_manifest.json` 位于 `stage_b_spatial_matched_eval_seed0/`，SHA256 `82bc5613dad71cf07eaf91138bd0da389195ce27bb1361b138bc62665d005bc7`；`stage_b_spatial_matched_metrics.json` SHA256 为 `2d2ca9c72e4aff84297eea108d7738f21513f9ed7d17c79b7b2e7e9311f8aa1d`。两项 training run 的 manifest/checkpoint/metrics hashes 见上表。
